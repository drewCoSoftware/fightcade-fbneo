...
size_t CGamepadDatabase::ParseLine(std::wstring& curLine, CGamepadButtonMapping& mapping, GUID& mappingGuid) {
  ...
  // TODO:
  // Parse the guid from the SDL version into something that we can use in fightcade.
  // Basically we want to compose a GUID that is compatible with our system.  From there we can
  // then parse out each of the button mappings.
  const size_t GUID_LEN = 32;
  auto sdlGuid = cols[0];     // Check this to make sure that it is the correct length / format.  If not, skip it!
  ...
  // TODO: Extra, unnecessary data copies here....
  GUID mGuid = TranslateSDLGuid(sdlGuid);
  memcpy(&mappingGuid, &mGuid, sizeof(GUID));


  // Make sure that the last input listed is 'platform'
  std::wstring platform = cols[colCount - 1];
  if (!platform._Equal(this->Platform.data())) {
    // Not the platform we care about.
    return WRONG_PLATFORM;
  }

  if (platform.substr(0, 8) != L"platform") {
    printf("Last entry is not listed as 'platform'... skipping...\n");
    return CANT_PARSE;
  }

  auto pParts = Split(platform, L":", false);
  if (pParts.size() != 2) {
    // VERBOSE:
    printf("Platform string could not be parsed!... skipping...\n");
    return CANT_PARSE;
  }
  size_t useLen = (std::min)(pParts[1].length(), (size_t)(MAX_NAME - 1));
  memcpy(mapping.Platform, pParts[1].data(), sizeof(wchar_t) * useLen);
  mapping.Platform[useLen] = 0;
...
void CGamepadDatabase::CreateDefaultMapping(){
  ...
  mapping.Entries[0] = { GPINPUT_LSTICK_Y, ITYPE_FULL_ANALOG, 0x02 };
  mapping.Entries[1] = { GPINPUT_LSTICK_X, ITYPE_FULL_ANALOG, 0x01 };

  // NOTE: These may be incorrect.
  mapping.Entries[2] = { GPINPUT_RSTICK_Y, ITYPE_FULL_ANALOG, 0x02 };
  mapping.Entries[3] = { GPINPUT_RSTICK_X, ITYPE_FULL_ANALOG, 0x01 };

  mapping.Entries[4] = { GPINPUT_DPAD_UP, ITYPE_DPAD, 0x02 };
  mapping.Entries[5] = { GPINPUT_DPAD_DOWN, ITYPE_DPAD, 0x03 };
  mapping.Entries[6] = { GPINPUT_DPAD_LEFT, ITYPE_DPAD, 0x00 };
  mapping.Entries[7] = { GPINPUT_DPAD_RIGHT, ITYPE_DPAD, 0x01 };

  mapping.Entries[8] = { GPINPUT_BACK, ITYPE_BUTTON, 0x06 };
  mapping.Entries[9] = { GPINPUT_START, ITYPE_BUTTON, 0x07 };

  mapping.Entries[10] = { GPINPUT_X, ITYPE_BUTTON, 0x02 };
  mapping.Entries[11] = { GPINPUT_Y, ITYPE_BUTTON, 0x03 };
  mapping.Entries[12] = { GPINPUT_A, ITYPE_BUTTON, 0x00 };
  mapping.Entries[13] = { GPINPUT_B, ITYPE_BUTTON, 0x01 };

  // ?? Maybe......
  mapping.Entries[14] = { GPINPUT_LEFT_BUMPER, ITYPE_BUTTON, 0x04 };
  mapping.Entries[15] = { GPINPUT_LEFT_TRIGGER, 	ITYPE_HALF_ANALOG, 0x05 };

  mapping.Entries[16] = { GPINPUT_RIGHT_BUMPER, ITYPE_BUTTON, 0x05 };
  mapping.Entries[17] = { GPINPUT_RIGHT_TRIGGER, ITYPE_HALF_ANALOG, 0x04 };

  mapping.EntryCount = 18;
